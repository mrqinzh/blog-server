...
import com.mrqinzh.webapp.auth.processor.AuthenticationProcessor;
import com.mrqinzh.webapp.auth.core.SecurityProperties;
import com.mrqinzh.webapp.auth.exp.AuthException;
import com.mrqinzh.webapp.auth.processor.AuthenticationPrePostProcessor;
import com.mrqinzh.webapp.auth.handler.DefaultLoginHandler;
...
public abstract class AbsAuthenticationFilter extends OncePerRequestFilter {
    ...
    @Autowired
    private List<AuthenticationPrePostProcessor> prePostAuthHandlers;
    ...
    protected boolean executeLogin(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            // 登录前置处理
            for (AuthenticationPrePostProcessor handler : prePostAuthHandlers) {
